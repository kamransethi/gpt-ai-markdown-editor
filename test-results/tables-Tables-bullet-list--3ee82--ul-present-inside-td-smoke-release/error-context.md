...
- Name: tables.spec.ts >> Tables >> bullet list in cell — <ul> present inside <td> @smoke
- Location: src\__tests__\playwright\tables.spec.ts:128:7
...
Locator: locator('.ProseMirror table td').first().locator('ul')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.ProseMirror table td').first().locator('ul')
