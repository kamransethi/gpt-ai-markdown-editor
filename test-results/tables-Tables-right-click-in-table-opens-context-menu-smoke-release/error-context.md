...
- Name: tables.spec.ts >> Tables >> right-click in table opens context menu @smoke
- Location: src\__tests__\playwright\tables.spec.ts:205:7
...
Locator: locator('.context-menu, .table-context-menu, [class*="context-menu"]').first()
Expected: visible
Timeout: 3000ms
...
Call log:
  ...
  - waiting for locator('.context-menu, .table-context-menu, [class*="context-menu"]').first()
...
> 211 |     await expect(menu.first()).toBeVisible({ timeout: 3000 });
  ...
  299 |   });
  300 | 
  301 |   test('table with header row — th elements present for first row', async ({ page }) => {
  302 |     await runEditorCommand(page, 'insertTable', { rows: 3, cols: 3, withHeaderRow: true });
  303 |     await expect(page.locator('.ProseMirror table th')).toHaveCount(3);
  304 |   });
  305 | });
  306 | 
```