...
(() => {
  ...
  function findDiffStart(a, b2, pos) {
    for (let i = 0; ; i++) {
      ...
      if (childA.isText && childA.text != childB.text) {
        for (let j2 = 0; childA.text[j2] == childB.text[j2]; j2++) pos++;
        return pos;
      }
  ...
  function compareDeep(a, b2) {
    ...
    if (Array.isArray(b2) != array) return false;
    if (array) {
      if (a.length != b2.length) return false;
      for (let i = 0; i < a.length; i++) if (!compareDeep(a[i], b2[i])) return false;
    } else {
      for (let p in a) if (!(p in b2) || !compareDeep(a[p], b2[p])) return false;
      for (let p in b2) if (!(p in a)) return false;
    }
    return true;
  }
  var Mark = class _Mark2 {
    ...
    */
    static sameSet(a, b2) {
      if (a == b2) return true;
      if (a.length != b2.length) return false;
      for (let i = 0; i < a.length; i++) if (!a[i].eq(b2[i])) return false;
      return true;
    }
    /**
    ...
    */
    check() {
      ...
      if (!Mark.sameSet(copy2, this.marks))
        throw new RangeError(
          `Invalid collection of marks for node ${this.type.name}: ${this.marks.map(m2 => m2.type.name)}`
        );
    ...
    compatible(other) {
      for (let i = 0; i < this.next.length; i++)
        for (let j2 = 0; j2 < other.next.length; j2++)
          if (this.next[i].type == other.next[j2].type) return true;
      return false;
    }
    /**
    ...
    toString() {
      ...
      function scan(m2) {
        ...
        for (let i = 0; i < m2.next.length; i++)
          if (seen.indexOf(m2.next[i].next) == -1) scan(m2.next[i].next);
      }
      scan(this);
      return seen
        .map((m2, i) => {
    ...
    */
    constructor(spec) {
      ...
      for (let prop in this.marks) {
        ...
        type.excluded =
          excl == null ? [type] : excl == '' ? [] : gatherMarks(this, excl.split(' '));
      }
      ...
      for (let i = after ? this.styles.indexOf(after) + 1 : 0; i < this.styles.length; i++) {
        ...
        if (
          ...
          (style2.length > prop.length &&
            (style2.charCodeAt(prop.length) != 61 || style2.slice(prop.length + 1) != value))
        )
          continue;
        if (rule.getAttrs) {
    ...
    */
    static fromSchema(schema) {
      return (
        ...
        (schema.cached.domParser = new _DOMParser(schema, _DOMParser.schemaRules(schema)))
      );
    }
  };
  var blockTags = {
    ...
    finish(openEnd) {
      if (!(this.options & OPT_PRESERVE_WS)) {
        ...
        if (last && last.isText && (m2 = /[ \t\r\n\u000c]+$/.exec(last.text))) {
          let text = last;
          if (last.text.length == m2[0].length) this.content.pop();
          else
            this.content[this.content.length - 1] = text.withText(
              text.text.slice(0, text.text.length - m2[0].length)
            );
        }
      }
  ...
  var ParseContext = class {
        ...
        for (let i = 0; i < this.parser.matchedStyles.length; i++) {
          ...
          if (value)
            for (let after = void 0; ; ) {
              ...
              if (rule.clearMark) marks = marks.filter(m2 => !rule.clearMark(m2));
    ...
    get currentPos() {
      ...
      for (let i = this.open; i >= 0; i--) {
        ...
        for (let j2 = content.length - 1; j2 >= 0; j2--) pos += content[j2].nodeSize;
        if (i) pos++;
      }
      return pos;
    }
  ...
  function renderSpec(doc3, structure, xmlNS, blockArraysIn) {
    if (typeof structure == 'string') return { dom: doc3.createTextNode(structure) };
  ...
  var ReplaceAroundStep = class _ReplaceAroundStep extends Step {
    ...
    */
    ...
    apply(doc3) {
      if (
        this.structure &&
        (contentBetween(doc3, this.from, this.gapFrom) || contentBetween(doc3, this.gapTo, this.to))
      )
      ...
      if (toRemove && toRemove.length) {
        ...
        for (let i = 0; i < toRemove.length; i++) {
          ...
          for (let j2 = 0; j2 < matched.length; j2++) {
            let m2 = matched[j2];
            if (m2.step == step - 1 && style2.eq(matched[j2].style)) found2 = m2;
          }
          if (found2) {
            found2.to = end;
            found2.step = step;
  ...
  function canSplit(doc3, pos, depth = 1, typesAfter) {
    ...
    return $pos
      .node(base2)
      .canReplaceWith(index, index, baseType ? baseType.type : $pos.node(base2 + 1).type);
  }
  ...
  function canAppendWithSubstitutedLinebreaks(a, b2) {
    if (!b2.content.size) a.type.compatibleContent(b2.type);
  ...
  function replaceRange(tr2, from2, to, slice2) {
    ...
    if (fitsTrivially($from, $to, slice2)) return tr2.step(new ReplaceStep(from2, to, slice2));
  ...
  function closeFragment(fragment, depth, oldOpen, newOpen, parent) {
    if (depth < oldOpen) {
      ...
      fragment = fragment.replaceChild(
        0,
        first2.copy(closeFragment(first2.content, depth + 1, oldOpen, newOpen, first2))
      );
    }
    if (depth > newOpen) {
  ...
  function deleteRange(tr2, from2, to) {
    ...
    for (let i = 0; i < covered.length; i++) {
      ...
      if (
        depth > 0 &&
        (last ||
          $from.node(depth - 1).canReplace($from.index(depth - 1), $to.indexAfter(depth - 1)))
      )
  ...
  var TextSelection = class _TextSelection extends Selection {
    ...
    */
    ...
    eq(other) {
      return (
        other instanceof _TextSelection && other.anchor == this.anchor && other.head == this.head
      );
    }
    getBookmark() {
    ...
    */
    static between($anchor, $head, bias) {
      ...
      if (!$head.parent.inlineContent) {
        let found2 =
          Selection.findFrom($head, bias, true) || Selection.findFrom($head, -bias, true);
  ...
  var Transaction = class extends Transform {
    ...
    */
    get isGeneric() {
      for (let _2 in this.meta) return false;
      return true;
    }
    /**
  ...
  var EditorState = class _EditorState {
    ...
    */
    applyInner(tr2) {
      if (!tr2.before.eq(this.doc)) throw new RangeError('Applying a mismatched transaction');
    ...
    */
    toJSON(pluginFields) {
      ...
      if (this.storedMarks) result.storedMarks = this.storedMarks.map(m2 => m2.toJSON());
          ...
          if (dispatch) {
            ...
            tr2.setSelection(
              textblockAt(before, 'end')
                ? Selection.findFrom(tr2.doc.resolve(tr2.mapping.map($cut.pos, -1)), -1)
  ...
  var joinUp = (state, dispatch) => {
    ...
    if (dispatch) {
      ...
      if (nodeSel)
        tr2.setSelection(
          NodeSelection.create(tr2.doc, point - state.doc.resolve(point).nodeBefore.nodeSize)
        );
      dispatch(tr2.scrollIntoView());
    }
    return true;
  ...
  function liftOutOfList(state, dispatch, range) {
    ...
    if (tr2.mapping.map(range.end) != range.start + $start.nodeAfter.nodeSize) return false;
  ...
  function storeScrollPos(view) {
    ...
    let refDOM, refTop;
    for (
      let x2 = (rect.left + rect.right) / 2, y2 = startY + 1;
      y2 < Math.min(innerHeight, rect.bottom);
      y2 += 5
    ) {
    ...
    if (prev.style != cur.style) {
      if (prev.style) {
        let prop = /\s*([\w\-\xa1-\uffff]+)\s*:(?:"(?:\\.|[^"])*"|'(?:\\.|[^'])*'|\(.*?\)|[^;])*/g,
          m2;
        while ((m2 = prop.exec(prev.style))) dom.style.removeProperty(m2[1]);
      }
  ...
  function sameOuterDeco(a, b2) {
    if (a.length != b2.length) return false;
    for (let i = 0; i < a.length; i++) if (!a[i].type.eq(b2[i].type)) return false;
    return true;
  }
  function rm(dom) {
  ...
  function captureKeyDown(view, event) {
    ...
    if (code == 8 || (mac2 && code == 72 && mods == 'c')) {
      ...
      return (
        safariDownArrowBug(view) || selectVertically(view, 1, mods) || skipIgnoredNodes(view, 1)
      );
    } else if (
      mods == (mac2 ? 'm' : 'c') &&
  ...
  function addToSibling(wrap2, lastWrap, node, sibling, depth) {
    if (depth < wrap2.length && depth < lastWrap.length && wrap2[depth] == lastWrap[depth]) {
      ...
      if (match.matchType(depth == wrap2.length - 1 ? node.type : wrap2[depth + 1]))
        return sibling.copy(
          sibling.content.append(Fragment.from(withWrappers(node, wrap2, depth + 1)))
        );
    }
  }
  ...
  function closeSlice(slice2, openStart, openEnd) {
    if (openStart < slice2.openStart)
      slice2 = new Slice(
        closeRange(slice2.content, -1, openStart, slice2.openStart, 0, slice2.openEnd),
        openStart,
        slice2.openEnd
      );
  ...
  function defaultTripleClick(view, inside, event) {
    ...
    for (let i = $pos.depth + 1; i > 0; i--) {
      ...
      if (node.inlineContent)
        updateSelection(
          view,
          TextSelection.create(doc3, nodePos + 1, nodePos + 1 + node.content.size),
          'pointer'
        );
    ...
    if (!view.composing) {
      ...
      if (
        ...
        (state.storedMarks ||
          (!$pos.textOffset &&
            $pos.parentOffset &&
            $pos.nodeBefore.marks.some(m2 => m2.type.spec.inclusive === false)) ||
  ...
  function sliceSingleNode(slice2) {
    return slice2.openStart == 0 && slice2.openEnd == 0 && slice2.content.childCount == 1
  ...
  function handleDrop(view, event, dragging) {
    ...
    if (isNode) tr2.replaceRangeWith(pos, pos, slice2.content.firstChild);
  ...
  function compareObjs(a, b2) {
    if (a == b2) return true;
    for (let p in a) if (a[p] !== b2[p]) return false;
    for (let p in b2) if (!(p in a)) return false;
    return true;
  }
    ...
    locals(node) {
      ...
      for (let i = 0; i < this.members.length; i++) {
        ...
        if (!result) {
          ...
          for (let j2 = 0; j2 < locals.length; j2++) result.push(locals[j2]);
        }
      }
      ...
      switch (members.length) {
        ...
        default:
          return new _DecorationGroup(
            members.every(m2 => m2 instanceof DecorationSet)
              ? members
              : members.reduce(
                  (r, m2) => r.concat(m2 instanceof DecorationSet ? m2 : m2.members),
                  []
                )
          );
      }
  ...
  function mapChildren(oldChildren, newLocal, mapping, node, offset, oldOffset, options) {
    ...
    if (mustRebuild) {
      ...
      for (let i = 0, j2 = 0; i < built.children.length; i += 3) {
        ...
        while (j2 < children.length && children[j2] < from2) j2 += 3;
  ...
  var DOMObserver = class {
      ...
      this.observer =
        ...
        new window.MutationObserver(mutations => {
          ...
          if (
            ...
            mutations.some(
              m2 =>
                (m2.type == 'childList' && m2.removedNodes.length) ||
                (m2.type == 'characterData' && m2.oldValue.length > m2.target.nodeValue.length)
            )
          ) {
            this.flushSoon();
          } else if (
            safari &&
            view.composing &&
            mutations.some(m2 => m2.type == 'childList' && m2.target.nodeName == 'TR')
          ) {
    ...
    flush() {
      ...
      if (
        ...
        if (brs.length == 2) {
          let [a, b2] = brs;
          if (a.parentNode && a.parentNode.parentNode == b2.parentNode) b2.remove();
          else a.remove();
        } else {
        ...
        if (nextCell) {
          ...
          for (;;) {
            ...
            if (
              !first2 ||
              first2.nodeType != 1 ||
              first2.contentEditable == 'false' ||
              /^(BR|IMG)$/.test(first2.nodeName)
            )
              break;
            parent = first2;
          }
  ...
  function readDOMChange(view, from2, to, typeOver, addedNodes) {
    ...
    if (
      ...
      parse.doc.textBetween(change.start - parse.from - 1, change.start - parse.from + 1) == ' \xA0'
    ) {
      change.start--;
      change.endA--;
      change.endB--;
    ...
    if (
      ...
      looksLikeBackspace(doc3, change.start, change.endA, $from, $to) &&
  ...
  function keyName(event) {
    var ignoreKey =
      (mac3 && event.metaKey && event.shiftKey && !event.ctrlKey && !event.altKey) ||
    ...
    return function (view, event) {
      ...
      if (name.length == 1 && name != ' ') {
        ...
        if (
          ...
          !(windows2 && event.ctrlKey && event.altKey) &&
  ...
  var focus =
    ...
    ({ editor: editor2, view, tr: tr2, dispatch }) => {
      options = {
        scrollIntoView: true,
        ...options,
      };
  ...
  var insertContentAt =
    ...
    ({ tr: tr2, dispatch, editor: editor2 }) => {
      var _a;
      if (dispatch) {
        options = {
        ...
        const emitContentError = error => {
          editor2.emit('contentError', {
            ...
            disableCollaboration: () => {
              if (
                'collaboration' in editor2.storage &&
                typeof editor2.storage.collaboration === 'object' &&
                editor2.storage.collaboration
              ) {
        ...
        if (
          ...
          !editor2.options.enableContentCheck &&
          editor2.options.emitContentError
        ) {
          try {
  ...
  var setContent =
    ...
    ({ editor: editor2, tr: tr2, dispatch, commands }) => {
      const { doc: doc3 } = tr2;
  ...
  function isExtensionRulesEnabled(extension, enabled) {
    if (Array.isArray(enabled)) {
      return enabled.some(enabledExtension => {
        const name =
          typeof enabledExtension === 'string' ? enabledExtension : enabledExtension.name;
    ...
    ({ state, dispatch, chain }) => {
      ...
      return chain()
        ...
        .command(({ state: updatedState }) => {
          return setBlockType2(type, { ...attributesToCopy, ...attributes })(
            updatedState,
            dispatch
          );
        })
  ...
  var splitBlock2 =
    ({ keepMarks = true } = {}) =>
    ({ tr: tr2, state, dispatch, editor: editor2 }) => {
      ...
      const deflt =
        $from.depth === 0
          ? void 0
          : defaultBlockAt2($from.node(-1).contentMatchAt($from.indexAfter(-1)));
      let types =
        atEnd && deflt
          ? [
              {
      ...
      if (
        !types &&
        !can &&
        canSplit(tr2.doc, tr2.mapping.map($from.pos), 1, deflt ? [{ type: deflt }] : void 0)
      ) {
        can = true;
        types = deflt
          ? [
  ...
  var joinListForwards = (tr2, listType) => {
    ...
    const canJoinForwards =
      list.node.type === (nodeAfter == null ? void 0 : nodeAfter.type) && canJoin(tr2.doc, after);
    if (!canJoinForwards) {
      return true;
    }
    tr2.join(after);
  ...
  var toggleList =
    ...
    ({ editor: editor2, tr: tr2, state, dispatch, chain, commands, can }) => {
    ...
    static handleExit({ editor: editor2, mark }) {
      ...
      if (isAtEnd) {
        ...
        const isInMark = !!currentMarks.find(
          m2 => (m2 == null ? void 0 : m2.type.name) === mark.name
        );
        if (!isInMark) {
          return false;
        }
  ...
  function run2(config) {
    ...
    state.doc.nodesBetween(from2, to, (node, pos) => {
      ...
      const contentSize =
        (_e2 =
          (_d = (_c = node.content) == null ? void 0 : _c.size) != null ? _d : node.nodeSize) !=
        null
          ? _e2
          : 0;
  ...
  var Delete = Extension.create({
    ...
    onUpdate({ transaction, appendedTransactions }) {
      ...
      if (
        (_c =
          (_b = (_a = this.editor.options.coreExtensionOptions) == null ? void 0 : _a.delete) ==
          null
            ? void 0
            : _b.async) != null
          ? _c
          ...
          props: {
            handleDOMEvents: {
              focus: (view, event) => {
                ...
                const transaction = editor2.state.tr
                  .setMeta('focus', { event })
        ...
        this.editor.commands.first(({ commands }) => [
          ...
          () =>
            commands.command(({ tr: tr2 }) => {
              ...
              const $parentPos =
                $anchor.parent.isTextblock && pos > 0 ? tr2.doc.resolve(pos - 1) : $anchor;
  ...
  var Editor = class extends EventEmitter {
    ...
    createExtensionManager() {
      ...
      const coreExtensions = this.options.enableCoreExtensions
        ? [
            ...
            Tabindex.configure({
              value:
                (_d = (_c = this.options.coreExtensionOptions) == null ? void 0 : _c.tabindex) ==
                null
                  ? void 0
                  : _d.value,
            }),
    ...
    createView(element) {
      ...
      const transformPastedHTML =
        this.extensionManager.transformPastedHTML(baseTransformPastedHTML);
    ...
    $nodes(selector, attributes) {
      var _a;
      return (
        ((_a = this.$doc) == null ? void 0 : _a.querySelectorAll(selector, attributes)) || null
      );
    }
    $pos(pos) {
...
`,
    ...
    addCommands() {
      return {
        setHardBreak:
          () =>
          ({ commands, chain, state, editor: editor2 }) => {
      ...
      return {
        setHorizontalRule:
          ...
          ({ chain, state }) => {
            ...
            return currentChain
              .command(({ state: chainState, tr: tr2, dispatch }) => {
                if (dispatch) {
...
3. "-" cannot repeat`);
  ...
  var handleDelete = (editor2, name) => {
    ...
    if (nextListIsDeeper(name, editor2.state)) {
      return editor2
        .chain()
        .focus(editor2.state.selection.from + 4)
        .lift(name)
        .joinBackward()
        .run();
    }
    ...
    if (tr2.steps.length == 0) {
      ...
      if (appended.getMeta(historyKey).redo)
        return new HistoryState(
          history2.done.addTransform(tr2, void 0, options, mustPreserveItems(state)),
          history2.undone,
        ...
        return new HistoryState(
          history2.done,
          history2.undone.addTransform(tr2, void 0, options, mustPreserveItems(state)),
          null,
          history2.prevTime,
          history2.prevComposition
        );
      ...
      props: {
        handleDOMEvents: {
          beforeinput(view, e) {
            ...
            if (!command2 || !view.editable) return false;
            e.preventDefault();
  ...
  var Be = /(?!~)[\s\p{P}\p{S}]/u;
  var De = /(?:[^\s\p{P}\p{S}]|~)/u;
  var qe = k(/link|precode-code|html/, 'g')
    .replace(
      'link',
      /\[(?:[^\[\]`]|(?<a>`+)[^`]+\k<a>(?!`))*?\]\((?:\\[\s\S]|[^\\\(\)]|\((?:\\[\s\S]|[^\\\(\)])*\))*\)/
    )
  ...
  var Ue = k(/\\(punct)/, 'gu')
    .replace(/punct/g, z)
    .getRegex();
  var Ke = k(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/)
...
`),
              S = this.list(f);
            ((s[s.length - 1] = S),
              (r = r.substring(0, r.length - h2.raw.length) + S.raw),
...
`,
              ...
              if (
                ((h2 = Z),
                this.options.pedantic
                  ? ((h2 = h2.replace(this.rules.other.listReplaceNesting, '  ')), (A = h2))
...
`),
          ...
          (i.loose ||
            (a ? (i.loose = true) : this.rules.other.doubleBlankLine.test(p) && (a = true)),
            i.items.push({
              type: 'list_item',
              raw: p,
  ...
  var x = class u {
    ...
    constructor(e) {
      ...
      (this.options.pedantic
        ...
        : this.options.gfm &&
          ((t.block = B.gfm), this.options.breaks ? (t.inline = E.breaks) : (t.inline = E.gfm)),
      ...
      return (n, r) => {
        ...
        if ((s.hooks && ((s.hooks.options = s), (s.hooks.block = e)), s.async))
          return (async () => {
            ...
            let h2 = await (s.hooks ? await s.hooks.provideParser() : e ? b.parse : b.parseInline)(
              c,
              s
            );
...
Please report this to https://github.com/markedjs/marked.`),
  ...
  function findMarksToClose(currentMarks, nextNode) {
    ...
    Array.from(currentMarks.keys()).forEach(markType => {
      if (
        !nextNode ||
        !nextNode.marks ||
        !nextNode.marks.map(mark => mark.type).includes(markType)
      ) {
  ...
  function findMarksToCloseAtEnd(activeMarks, currentMarks, nextNode, markSetsEqual) {
    ...
    const nextNodeHasDifferentMarks =
      ...
      nextNode.type === 'text' &&
      nextNode.marks &&
      !markSetsEqual(currentMarks, new Map(nextNode.marks.map(mark => [mark.type, mark])));
  ...
  var MarkdownManager = class {
    ...
    constructor(options) {
      ...
      this.indentStyle =
        (_c = (_b = options == null ? void 0 : options.indentation) == null ? void 0 : _b.style) !=
        null
          ? _c
          : 'space';
      this.indentSize =
        (_e2 = (_d = options == null ? void 0 : options.indentation) == null ? void 0 : _d.size) !=
        null
          ? _e2
          : 2;
      this.baseExtensions = (options == null ? void 0 : options.extensions) || [];
      if (
        (options == null ? void 0 : options.markedOptions) &&
    ...
    registerExtension(extension) {
      ...
      const markdownCfg =
        (_a = getExtensionField(extension, 'markdownOptions')) != null ? _a : null;
      const isIndenting =
        (_b = markdownCfg == null ? void 0 : markdownCfg.indentsContent) != null ? _b : false;
      ...
      return tokens.flatMap((token, index) => {
        ...
        if (parseImplicitEmptyParagraphs && token.type === 'space') {
          const nextNonSpaceTokenIndex =
            (_a = nonSpaceTokenIndexes[nextNonSpaceTokenPointer]) != null ? _a : -1;
...
`);
    ...
    parseInlineTokens(tokens) {
      ...
      for (let i = 0; i < tokens.length; i += 1) {
        ...
        if (token.type === 'text') {
          ...
          const raw = (
            (_b = (_a = token.raw) != null ? _a : token.text) != null ? _b : ''
          ).toString();
    ...
    encodeTextForMarkdown(text, node, parentNode2) {
      const isInsideCode =
        ((parentNode2 == null ? void 0 : parentNode2.type) != null &&
          this.codeTypes.has(parentNode2.type)) ||
        (node.marks || []).some(m2 => this.codeTypes.has(typeof m2 === 'string' ? m2 : m2.type));
    ...
    renderNodeToMarkdown(node, parentNode2, index = 0, level = 0, meta = {}) {
      ...
      const previousNode =
        Array.isArray(parentNode2 == null ? void 0 : parentNode2.content) && index > 0
          ? parentNode2.content[index - 1]
          : void 0;
      const helpers = {
      ...
      const rendered =
        ((_a = handler.renderMarkdown) == null
          ? void 0
          : _a.call(handler, node, helpers, context)) || '';
      return rendered;
    }
    /**
        ...
        if (node.type === 'text') {
          ...
          if (hasCrossedBoundary) {
            const nextMarkTypes = new Set(
              ((nextNode == null ? void 0 : nextNode.marks) || []).map(mark => mark.type)
            );
            ...
            const activeMarksClosingHereLifo = activeMarksClosingHere
              .slice()
              .sort((a, b2) => activeMarkKeys.indexOf(b2) - activeMarkKeys.indexOf(a));
            marksToCloseAtEnd = [
          ...
          const afterMarkdown =
            node.type === 'hardBreak'
              ...
              : reopenMarksAfterNode(marksToReopen, activeMarks, (markType, mark) => {
                  var _a;
                  const openingMode =
                    (_a = openingModesToReopen.get(markType)) != null ? _a : 'markdown';
    ...
    getMarksToOpenForSerialization(activeMarks, currentMarks, nextNode) {
      ...
      const byRankInnerFirst = (a, b2) => {
        ...
        const rankB =
          (_b = this.extensionRanks.get(b2.type)) != null ? _b : Number.MAX_SAFE_INTEGER;
        if (rankA !== rankB) {
          return rankB - rankA;
        }
      ...
      const endingHere = marksToOpen
        .filter(mark => !nextMarkTypes.has(mark.type))
    ...
    addCommands() {
      return {
        setContent: (content, options) => {
          ...
          const actualContentType = assumeContentType(
            content,
            options == null ? void 0 : options.contentType
          );
  ...
  // src/webview/extensions/spellCheck.ts
  var NO_SCAN_TYPES = /* @__PURE__ */ new Set([
    'codeBlock',
    'code',
    'image',
    'hardBreak',
  ...
  var INITIAL_SCAN_CHUNK = 50;
  function normaliseQuotes(text) {
    return text.replace(/[\u2018\u2019]/g, "'");
  }
  function maskUrls(text) {
    return text.replace(/https?:\/\/\S+|ftp:\/\/\S+|www\.\S+|\S+@\S+\.\S+/gi, m2 =>
      ' '.repeat(m2.length)
    );
  }
  ...
  function isNoScanNode(node) {
    return NO_SCAN_TYPES.has(node.type.name);
  }
  function collectFragments(doc3, fromPos = 0, toPos = doc3.content.size) {
    const frags = [];
    doc3.nodesBetween(fromPos, toPos, (node, pos) => {
      if (isNoScanNode(node)) return false;
      if (node.isText) {
        if (node.marks.some(m2 => m2.type.name === 'code')) return;
  ...
  var spellCheckKey = new PluginKey('spellCheck');
  var worker = null;
  ...
  async function getOrCreateWorker() {
    ...
    const workerUrl = window.SPELLCHECK_WORKER_URL;
    if (!workerUrl) {
    ...
    worker.addEventListener('message', event => {
      ...
      if (msg.type === 'READY') {
        if (editorViewRef) {
          ...
          const tr2 = state.tr.setMeta(spellCheckKey, { workerReady: true });
  ...
  function postToWorker(message) {
    void getOrCreateWorker().then(w2 => {
      if (w2) w2.postMessage(message);
    });
  }
  function scheduleInitialScan(view) {
    const { doc: doc3 } = view.state;
    const frags = collectFragments(doc3);
    let offset = 0;
    ...
    pendingCallbacks.set(id, results => {
      ...
      for (const { key, errors } of results) {
        ...
        for (const err of errors) {
          const from2 = frag.docFrom + err.from;
          const to = frag.docFrom + err.to;
          if (from2 < 0 || to > (editorViewRef?.state.doc.content.size ?? 0)) continue;
          newDecorations.push(
            Decoration.inline(
              from2,
              to,
      ...
      const tr2 = state.tr.setMeta(spellCheckKey, { decorations: nextSet });
      editorViewRef.dispatch(tr2);
    });
  ...
  function createSpellCheckPlugin() {
    return new Plugin({
      ...
      state: {
        ...
        apply(tr2, value, _old, newState) {
          const meta = tr2.getMeta(spellCheckKey);
          if (meta?.decorations) {
            return { decorations: meta.decorations, workerReady: value.workerReady };
          }
          if (meta?.workerReady) {
            return {
              decorations: value.decorations.map(tr2.mapping, newState.doc),
              workerReady: true,
            };
          }
          const mapped = value.decorations.map(tr2.mapping, newState.doc);
        ...
        return {
          update(updatedView, prevState) {
            ...
            if (!updatedView.state.doc.eq(prevState.doc)) {
              const { from: from2 } = updatedView.state.selection;
              const $from = updatedView.state.doc.resolve(from2);
              const blockStart = from2 - $from.parentOffset;
  ...
  async function initSpellCheck(payload) {
  ...
  var SpellCheck = Extension.create({
    name: 'spellCheck',
    addProseMirrorPlugins() {
  ...
  // src/__tests__/playwright/harness/spell-harness.ts
  var mountEl = document.getElementById('editor');
  ...
  var affUrl = window.SPELL_AFF_URL;
  var dicUrl = window.SPELL_DIC_URL;
...
//# sourceMappingURL=spell-harness.js.map
