...
3. "-" cannot repeat`);
  ...
  var K = { blockquote: Le, code: Te, def: Se, fences: Oe, heading: we, hr: C, html: _e, lheading: ie3, list: $e, newline: Re, paragraph: oe, table: _, text: Pe };
...
`);
    ...
    return new Plugin({
      ...
      state: {
        ...
        apply(tr2, value, _old, newState) {
          ...
          if (meta?.workerReady) {
            return { decorations: value.decorations.map(tr2.mapping, newState.doc), workerReady: true };
          }
  ...
  window.spellAPI = {
    ...
    getSpellErrorWords() {
      const state = spellCheckKey.getState(editor.state);
      if (!state) return [];
      return state.decorations.find().map((d) => d.spec.word ?? "").filter(Boolean);
    }
  };
